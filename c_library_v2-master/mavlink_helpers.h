 ...
 * A typical use scenario of this function call is:
 *
 * 这是一个处理完整 MAVLink 解析的便捷函数。
 * 该函数将一次解析一个字节并返回一次完整的数据包
 * 可以成功解码。此函数将返回 0 或 1。
 *
 * 消息被解析到一个内部缓冲区（每个通道一个）。当一个完整的
 * 收到消息，它被复制到 *r_message 并且频道的状态是
 * 复制到 *r_mavlink_status。
 *
 * @param chan 要解析的频道ID。
 * 通道不是像串口那样的物理消息通道，而是一个逻辑分区
 * 通信流。 COMM_NB 是通道数的限制
 * 在 MCU（例如 ARM7）上，而 COMM_NB_HIGH 是 Linux/Windows 中通道数的限制
 * @param c 要解析的字符
 *
 * @param r_message 如果没有消息可以解码，则为 NULL，否则为消息数据
 * @param r_mavlink_status 如果消息被解码，则填充频道的统计信息
 * @return 0 如果没有消息可以被解码或坏 CRC，1 表示好消息和 CRC
 *
 * 该函数调用的一个典型使用场景是：
 * 
 * @code
